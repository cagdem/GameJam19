...
public class Game : MonoBehaviour
{
    ...
    void GotNumber(Room room, int n, float probability)
    {
        ...
        /*
        //Room listesine oda ekleyince ses olmadýðý için patlýyor
    ...
    void Update()
    {
        ...
        bool pause = Input.GetKey(KeyCode.P);
        bool reverse = Input.GetKey(KeyCode.R);
        bool resume = Input.GetKey(KeyCode.F);

        if (pause)
        {
            Debug.Log("Pause");
            PlaybackController.Instance.StartPlayback();
        }
        else if (reverse)
        {
            Debug.Log("Reverse");
            PlaybackController.Instance.StartReversing();
        }
        else if (resume)
        {
            Debug.Log("Resume");
